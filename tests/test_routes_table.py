...
def test_라우트_테이블이_스펙의_예순여덟_개와_스코프까지_정확히_같다():
    assert len(EXPECTED_ROUTES) == 124
    assert len(routes.ROUTES) == 124, "행 수가 다르다 — 중복 행이거나 빠진 행이다"
...
def test_소유자_라우트는_41_개_프로필_라우트는_27_개다():
    ...
    #   + 0.15.0 스킬 CRUD 11 + 스킬 Hub 6 + curator·관계도 8 + 0.17.0 MCP 21 + 0.18.0 승인 정책 4
    #   + 대화 중 묻기 3 = 80.
    assert by_scope == {routes.Scope.DEFAULT: 4 + 1 + 1 + 21 + 1 + 2 + 1 + 2 + 2 + 6 + 3,
                        routes.Scope.PROFILE: 5 + 12 + 2 + 2 + 4 + 2 + 11 + 6 + 8 + 21 + 4 + 3}
